...
class TestUserCRUD(TestCase):
    ...
    def test_create_url_generates_different_representation_for_the_same_url_each_time(self):
        _, representation_1 = self.crud.create_url(self.user.id, 'foo.bar.com')
        _, representation_2 = self.crud.create_url(self.user.id, 'foo.bar.com')
    ...
    def test_create_url_uses_custom_representation_if_not_already_used(self):
        _, representation = self.crud.create_url(self.user.id, 'foo.bar.com', custom_representation='custom')
    ...
    def test_create_url_uses_similar_custom_representation_in_case_of_conflict_in_order(self):
        _, representation_1 = self.crud.create_url(self.user.id, 'foo.bar.com', custom_representation='custom')
        for i in range(10):
            _, representation_i = self.crud.create_url(self.user.id, 'foo.bar.com', custom_representation='custom')
            self.assertEqual(representation_i, f'custom{i}')
        _, representation_00 = self.crud.create_url(self.user.id, 'foo.bar.com', custom_representation='custom')
    ...
    def test_create_url_uses_similar_custom_representation_in_case_of_conflict_without_order(self):
        self.crud.create_url(self.user.id, 'foo.bar.com', custom_representation='custom')
        self.crud.create_url(self.user.id, 'foo.bar.com', custom_representation='custom0')
        self.crud.create_url(self.user.id, 'foo.bar.com', custom_representation='custom2')
        _, representation = self.crud.create_url(self.user.id, 'foo.bar.com', custom_representation='custom')
        self.assertEqual('custom1', representation)

    def test_create_url_raises_value_error_given_invalid_custom_string(self):
        self.assertRaises(ValueError, self.crud.create_url, self.user.id, 'foo.bar.com', '!nv^l!d')

    def test_retrieve_url_finds_the_correct_url_given_short_representation(self):
        url_normal, representation_normal = self.crud.create_url(self.user.id, 'foo.bar.com')
        url_custom, representation_custom = self.crud.create_url(self.user.id, 'foo.bar.com',
